...
export default function AdminReports() {
  ...
  const { transactions, adRevenue, subscriptions } = useStore();
  ...
  const revenueData = React.useMemo(() => {
    ...
    // Simuler les revenus publicitaires
    last12Months.forEach(d => {
      d.ads = Math.round(adRevenue.lastMonth / 12);
    });

    return last12Months;
  }, [transactions, adRevenue]);
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-3">
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm">
          ...
          <p className="mt-2 text-3xl font-semibold text-gray-900">
            {adRevenue.lastMonth.toLocaleString()} FCFA
          </p>
        </div>
      </div>
    </div>
